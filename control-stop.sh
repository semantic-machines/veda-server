...
TELEGRAM_BOT_TOKEN=$(grep -v '^#' ./veda.properties | grep "tg_notify_token" | cut -d'=' -f2 | tr -d ' "')
TELEGRAM_CHAT_ID=$(grep -v '^#' ./veda.properties | grep "tg_notify_chat_id" | cut -d'=' -f2 | tr -d ' ')
STAND_NAME=$(grep -v '^#' ./veda.properties | grep "name" | cut -d'=' -f2 | tr -d ' "')
...
if stop_veda "${pids[@]}"; then
    ...
    send_telegram_message "✅ successfully stopped"
    rm -f -r .pids
else
