...
use std::{fmt, fs};
use v_common::ft_xapian::index_schema::IndexerSchema;
use v_common::ft_xapian::key2slot::{Key2Slot, XAPIAN_INFO_PATH};
    ...
    pub(crate) fn init(&mut self, use_db: &str) -> Result<(), XError> {
        ...
        for (db_name, path) in self.db2path.iter() {
            ...
            let mut db = if Path::new(full_path).is_dir() {
                ...
                WritableDatabase::new(path, DB_CREATE_OR_OPEN, CHERT)?
            };
