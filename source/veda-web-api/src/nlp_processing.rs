...
async fn transcribe(filepath: String, nlp_config: &NLPServerConfig) -> Result<String, actix_web::Error> {
    ...
    let file_content = fs::read(&output_path).await
    ...
    let response = client.post(&format!("{}/inference", nlp_config.whisper_server_url))
        .multipart(form)
        .send()
        .await
    ...
    let response_text = response.text().await
        .map_err(|e| actix_web::error::ErrorInternalServerError(format!("Failed to read Whisper.cpp response: {}", e)))?;

    let json: Value = serde_json::from_str(&response_text)
        .map_err(|e| actix_web::error::ErrorInternalServerError(format!("Failed to parse JSON response: {}", e)))?;
...
pub async fn augment_text(
    ...
    let prompt = format!("Улучши следующий текст для типа '{}' и свойства '{}': {}", payload.type_, payload.property, payload.text);
    ...
    let response = client
        .post(&nlp_config.llama_server_url)
        .json(&llama_request)
        .send()
        .await
    ...
    if !response.status().is_success() {
        return Err(actix_web::error::ErrorInternalServerError(format!("llama.cpp server returned error status: {}", response.status())));
    }

    let augmented_text = response.text().await.map_err(|e| actix_web::error::ErrorInternalServerError(format!("Failed to read llama.cpp response: {}", e)))?;

    crate::common::log(Some(&start_time), &uinf, "augment_text", "success", ResultCode::Ok);

    Ok(HttpResponse::Ok().content_type("text/plain").body(augmented_text))
}
