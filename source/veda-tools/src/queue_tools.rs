...
pub fn print_queue(queue_path: String, part_id: Option<u32>, out_type: Option<String>) {
    ...
    let print_type = if let Some(v) = out_type {
        if v.to_uppercase() == "TTL" {
            "TTL"
        } else {
            "JSON"
        }
    ...
    for _idx in 0..size_batch {
        ...
        if parse_raw(&mut queue_element).is_ok() {
            ...
            if !prev_state.is_empty() {
                if print_type == "TTL" {
                    println!("# prev_state --- ");
                    if let Ok(buf) = to_turtle(&[prev_state], &prefixes) {
                        println!("{}", String::from_utf8(buf).unwrap_or_default());
                    }
                } else if print_type == "JSON" {
            ...
            if !new_state.is_empty() {
                if print_type == "TTL" {
                    println!("# new_state --- ");
                    if let Ok(buf) = to_turtle(&[new_state], &prefixes) {
                        println!("{}", String::from_utf8(buf).unwrap_or_default());
                    }
                } else if print_type == "JSON" {
